...
public:
	...
	__device__ static float ray_intersect(const TriangleOctreeNode* nodes, int max_depth, const vec3& ro, const vec3& rd) {
		...
		// Based on the sign of the ray direction, setting mask order.
		...
		while (!query_stack.empty()) {
			...
			for (uint8_t aux = 0; aux < 8; ++aux) {
				// e.g. If ray directions are all positive, then reorder_mask = 111, aux0: 111, aux1: 110, ..., deeper child are processed first (further from the ray)
				...
				// Size of the bounding box reduces with the depth.
				BoundingBox bb = {size * vec3(pos), size * vec3(pos + u16vec3(1))};
				// testing the ray bounding box intersection.
				vec2 t = bb.ray_intersect(ro, rd);
				// return the intersection t.
